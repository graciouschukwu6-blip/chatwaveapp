...
router.post('/register', async (req, res) => {
  try {
    ...
    ).run(username, email, hashedPassword, chatNumber);
    ...
    res.status(201).json({ 
      ...
      user: { id: result.lastInsertRowid, username, email, chat_number: chatNumber },
      token 
    });
  } catch (err) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      user: { id: user.id, username: user.username, email: user.email, chat_number: user.chat_number, avatar: user.avatar },
      token 
    });
  } catch (err) {
    res.status(500).json({ error: 'Server error: ' + err.message });
